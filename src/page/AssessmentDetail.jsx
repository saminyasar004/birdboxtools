...
const Assessmendivetail = ({
	...
	const [checkedItems, setCheckedItems] = useState({
			...
			(acc, key, index) => ({
				...
				[`charisma-left-${index}-yes`]:
					...
					key === "Facial expressions"
						? initialData.TransformationalElements.CHARISMA[key] ===
						  true
						: false,
		...
		...Object.entries({
			...
			"Intellectual Stimulation":
				initialData.TransformationalElements[
					"Intellectual Stimulation"
				],
			"Idolized Influences":
		...
		...Object.keys(initialData.SensoryCoaching).reduce(
			(acc, key, index) => ({
				...acc,
				[`sensory-${index}-yes`]:
					initialData.SensoryCoaching[key] === true,
				[`sensory-${index}-no`]:
					initialData.SensoryCoaching[key] === false,
			}),
			{}
		),
		...Object.keys(initialData.Personalities["Fast Thinkers"]).reduce(
			(acc, key, index) => ({
				...
				[`personalities-1-${index}-yes`]:
					key !== "Fast Thinkers"
						? initialData.Personalities["Fast Thinkers"][key] ===
						  true
						: false,
		...
		...Object.keys(initialData.Personalities["Slow Thinkers"]).reduce(
			(acc, key, index) => ({
				...
				[`personalities-2-${index}-yes`]:
					key !== "Slow Thinkers"
						? initialData.Personalities["Slow Thinkers"][key] ===
						  true
						: false,
	...
	const resetForm = () => {
		...
		setCheckedItems({
			...
			...Object.keys(
				initialData.TransformationalElements.CHARISMA
			).reduce(
				(acc, key, index) => ({
					...acc,
		...
		return {
			...
			CoachingStyle: {
				...
				"Empty reps": checkedItems["coaching-right-0-yes"]
					...
					? false
					: null,
				"Individual considerations": checkedItems[
					"coaching-right-1-yes"
				]
					? true
			...
			TransformationalElements: {
				CHARISMA: {
					...
					"Facial expressions": checkedItems["charisma-left-4-yes"]
						...
						? false
						: null,
					"Use of Pauses and Silences": checkedItems[
						"charisma-right-0-yes"
					]
						? true
				...
				"Coaching philosophy": checkedItems["transformational-0-yes"]
					...
					? false
					: null,
				"Intellectual Stimulation": checkedItems[
					"transformational-1-yes"
				]
					? true
			...
			Personalities: {
				...
				"Slow Thinkers": {
					...
					"Systematic, deep thinkers, will ask lots of questions":
						checkedItems["personalities-2-1-yes"]
							? true
